...
import { ValidatePersonInfo, clearValidatePersonInfoSuccess, disableBtnsGlobal, setSuccess } from '../../redux/slices/existanceSlice'
...
export const FamilyMembers = ({ current, steps, setCurrent, next, prev, form }: any) => {
    ...
    const familyMember = () => {
        ...
        <Col className='vertical-end h65 ' xs={24} sm={24} md={8} lg={8} xl={8} xxl={8}>
            <Button onClick={syncFamilyMember}
                        ...
                        <>
                            {fields.map(({ key, name, ...restField }) => (
                                <Row gutter={[16, 24]} className='w100 mt24'>
                                    ...
                                    <Col className='vertical-end h65 ' xs={24} sm={24} md={8} lg={8} xl={8} xxl={8}>
                                        <Button onClick={() => syncPerson(key)}
                                            className={`addFamilyBtnSync ${disableSyncBtns[key] ? 'class' : ''} `} icon={<SyncOutlined className='syncIcon' />}>
                                            პიროვნების გადამოწმება</Button>
                                        <Button
                                            onClick={() => remove(name)} className={`addFamilyBtn deleteFamilyBtn ml10 trueClassRemove ${isFamilyMember ?  'remove' + key : ''} `}
                            ...
                            <Row className='addbtnRow'>
                                <Col xs={24} sm={24} md={8} lg={8} xl={8} xxl={8} onClick={addBtn}>
                                    <Form.Item className='mt24'>
                                        <Button  className='addFamilyBtn' onClick={() => add()} icon={<PlusCircleOutlined className='plusIcon' />}>დამატება</Button>
                                    </Form.Item>
                                </Col>
                            </Row>
                        </>
                ...
                <div className='w100 horizontal-center mt80'>
                    ...
                    {current < steps.length - 1 && (
                        <Form.Item className='widthOfSend'>
                            <Button disabled={!validatePersonInfoSuccess } htmlType="submit" className='stepNext'>
                                შემდეგი
                            </Button>
                        </Form.Item>
